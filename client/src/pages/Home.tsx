...
const Home = () => {
  ...
  // type arr = {id:number, style:string}
  // const arr:arr[] = []
  // testimonials.forEach(item=>arr.push({id:item.id, style:'hidden'}))
  // const arr = testimonials.map((item=>{
  //   if(item.id<4) return({id:item.id, style:''})
  //   else return({id:item.id, style:'hidden'})
  // }))
  // const [reviewArr, setReviewArr] = useState(arr)

  // const handleMoveLeft = ()=>{
  //   setReviewArr(prev =>{return(prev.map((item)=>{
  //     if(item.id===reviewIndex) return({...item, style:'S'})
  //     else if (item.id===reviewIndex+1) return ({...item, style:''})
  //     else if (item.id===reviewIndex+2) return ({...item, style:''})
  //     else if (item.id===reviewIndex+3) return ({...item, style:'hidden'})
  //     else return(item)
  //   }))})
  //   reviewIndex>0?setReviewIndex(reviewIndex-1):setReviewIndex(0)
  // }
  // const handleMoveRight = ()=>{
  //   setReviewArr(prev =>{return(prev.map((item)=>{
  //     if(item.id===reviewIndex) return({...item, style:'hidden'})
  //     else if (item.id===reviewIndex+1) return ({...item, style:''})
  //     else if (item.id===reviewIndex+2) return ({...item, style:''})
  //     else if (item.id===reviewIndex+3) return ({...item, style:''})
  //     else return(item)
  //     // return(item.id===reviewIndex?{...item, style:'hidden'}:item.id===reviewIndex+3?{...item, style:''}:item)
  //   }))})
  ...
  return (
    <div>
      <section>
        <div className='max-w-full mx-auto bg-gray-50 mt-[90px] mb-10'>
      ...
      <section id="testimonials" className="p-6 bg-myDarkBlue">
        <div className="max-w-5xl mx-auto flex flex-col items-start justify-center">
          ...
          <div className="flex flex-col sm:flex-row">
            {/* <button onClick={handleMoveLeft} disabled={reviewIndex>0?false:true} className=" bg-myBlue">Left</button> */}
            <ul className="list-none mx-auto flex flex-col sm:flex-row items-center gap-8 flex-wrap md:flex-nowrap">
              {testimonials.map((item, key)=>{
                return(
                  <li key={key} className={`w-11/12 sm:w-5/6 flex flex-col items-center md:py-6 px-2 rounded-3xl shadow-xl shadow-slate-900`}>
            ...
            {/* <button onClick={handleMoveRight} disabled={reviewArr.length-reviewIndex<3} className=" bg-myBlue">Right</button> */}
          </div>
